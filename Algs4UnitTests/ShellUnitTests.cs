...
namespace Algs4UnitTests
{
   ...
   public class ShellUnitTests
   {
      ...
      public void ShellTiny()
      {
         ...
         string[] actualResult = CommonSortUnitTests.SortCommon("tiny.txt", Shell.Instance);
      ...
      public void ShellWords3()
      {
         ...
         string[] actualResult = CommonSortUnitTests.SortCommon("words3.txt", Shell.Instance);
         for (int i = 0; actualResult.Length > i; i++)
         {
            Assert.AreEqual(expectedResult[i], actualResult[i]);
         }
